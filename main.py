...
def getRule():
    ...
    nowRule = 0
    for rule in KB:
        for premise in rule[0:len(rule)-1]: #เอา Then ออก
            print("in"+str(nowPremise))
            print("in"+str(len(rule) - 2))

            if premise not in BlackBoard:
                if premise in StartingNode:
                    print(str(premise)+" ? (y = yes, n = No)")
                    response = input()
                    if str(response) == "y":
                        print(nowPremise)
                        print(len(rule) - 2)
                        if nowPremise == len(rule) - 2:
                            # firetherule
                            for premise2 in rule:
                                BlackBoard.append(premise2)
                            KB.pop(nowRule)
                            return
                        else:
                            nowPremise = nowPremise + 1
    ...
    for BB in BlackBoard:
        for thermi in TherminalNode:
            if str(BB) == str(thermi):
                Concluding.append(BB)
    if Concluding != []:
        print("It is "+str(Concluding))
    else:
        print("This knowledge cannot solve your problem.")
    HaveRule = False
